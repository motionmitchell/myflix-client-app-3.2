...
export function setMovies(value) {
	console.log("reducer.setMovies: ", value)
	alert("setMovies");
...
export const addToFavorites = createAsyncThunk(
	...
	async (payload) => {
		//	alert("addToFavorites: " + payload.id);
		const resp = await fetch(server + 'user/movie/add/' + payload.id);
		if (resp.ok) {
			const msg = await resp.json();

			console.log("addToFavorites.fetch", msg);
			return { msg };
		}
	}
);
